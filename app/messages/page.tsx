...
export default function MessagesPage() {
  ...
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > 0) {
      ...
      imageFiles.forEach((file) => {
        ...
        reader.onload = (e) => {
          if (e.target?.result) {
            setSelectedFiles((prev) => [...prev, { file, preview: e.target!.result as string }])
          }
        }
  ...
  const handleSendMessage = () => {
    ...
    const newMessages: typeof initialMessages = []
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-background">
      ...
      <main className={cn("flex-1 flex flex-col bg-background relative min-h-0 h-full", isSidebarOpen ? "hidden md:flex" : "flex")}>
        ...
        {/* Messages */}
        <ScrollArea className="flex-1 p-4 min-h-0" viewportRef={scrollAreaRef}>
